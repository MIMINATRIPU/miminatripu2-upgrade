  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div>
          <span className="font-mono text-xs uppercase tracking-[0.25em] text-primary">{String(i + 1).padStart(2, "0")} / 06</span>
          <h3 className="mt-3 font-display font-black text-3xl sm:text-4xl tracking-tight text-gradient leading-none">
            {m.name}
          </h3>
          <p className="mt-1 text-sm text-foreground/60">{m.role}</p>
        </div>
        <span className="text-4xl group-hover:animate-float-slow">{m.emoji}</span>
      </div>
